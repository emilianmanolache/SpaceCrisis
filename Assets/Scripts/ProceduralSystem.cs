...
public class ProceduralSystem : MonoBehaviour
{
    ...
    public Texture ProminenceTexture = new Texture();

    public int moonsRange;
	...
	public void AddStar()
	{
		var gameObject   = AddBasicGameObject("Star", transform, 0.0f, 0.0f, 0.0f, 10.0f, 3.0f, 3.0f);
        
        var newStar = (GameObject) Instantiate(starPrefab, new Vector3(0.0f, 0.0f, 0.0f), gameObject.transform.rotation);
        var solarSystem = GameObject.FindGameObjectsWithTag("SolarSystem")[0];
        newStar.transform.parent = solarSystem.transform;
        newStar.transform.localScale = gameObject.transform.localScale;
	...
	{
		var gameObject   = AddBasicGameObject("Planet", transform, 15.0f, 30.0f, -3.0f, 3.0f, 0.5f, 1.0f);
		...
		// Add moons?
		/*for (var i = Random.Range(0, moonsRange); i >= 0; i--)
		{
